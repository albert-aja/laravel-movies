...
@section('title')
    {{ $detail['name'] }} ({{ $detail['release_year'] }})
@endsection
...
@section('content')
    <div class="header-info flex bg-no-repeat bg-cover bg-center items-center" style="background-image:radial-gradient(rgba(11, 23, 34, 0.8) 0%, rgba(8, 24, 39, 0.8) 35%, rgb(17, 24, 39) 85% ), url({{ $detail['backdrop_path'] }})">
        ...
        <div class="container mx-auto px-4 py-12 flex flex-col items-center md:flex-row md:grid md:grid-cols-4 z-10">
            <img src="{{ $detail['poster_path'] }}" alt="{{ $detail['name'] }}" class="w-full rounded-lg">
            <div class="mt-3 md:ml-24 md:mt-0 md:col-start-2 md:col-span-3">
                <h2 class="text-4xl font-semibold">{{ $detail['name'] }} ({{ $detail['release_year'] }})</h2>
                ...
                <div class="mt-10">
                    <div class="flex mt-4">
                        @foreach ($detail['created_by'] as $crew)
                            <div class="mr-12">
                                <div>{{ $crew['name'] }}</div>
                ...
                <div class="mt-10">
                    <h4 class="text-white font-bold text-lg">Networks</h4>
                     <div class="grid grid-cols-7 gap-5">
                        @foreach ($detail['networks'] as $network)
                        <div class="mt-4">
                            <img src="{{ $network['logo_path'] }}" alt="{{ $network['name'] }}" class="w-full">
                ...
                @foreach ($detail['images'] as $image)
                    <div class="mt-8">
                        <a
                            ...
                            href="#"
                        >
                            <img src="{{ config('services.tmdb.img').$image['file_path'] }}" alt="{{ $detail['name'] }}" class="hover:opacity-75 transition ease-in-out duration-150 rounded-lg">
                        </a>
                    </div>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-5">
                @foreach ($detail['recommendations'] as $show)
                    ...
                    <a href="{{ route('tv.show', $show['id']) }}">
                        <img src="{{ $show['poster_path'] }}" alt="{{ $show['name'] }}" class="hover:opacity-75 transition ease-in-out duration-150 rounded-md w-full">
                        <p class="text-sm mt-2 w-11/12 text-center">
                            {{ $show['name'] }}
                        </p>
                    </a>
